...
# Modern nvblox_torch (nvidia-isaac/nvblox, pip-installable >= 0.0.8)
from nvblox_torch import (
    Mapper,
    MapperParams,
...
class NvbloxPanoTSDF:
    def __init__(self, voxel_size_m=0.01, max_depth=6.0, face_size=512, device="cuda"):
        ...
        # Configure max integration distance
        proj_params = ProjectiveIntegratorParams()
        proj_params.projective_integrator_max_integration_distance_m = float(max_depth)
        ...
        # Camera(fu, fv, cu, cv, width, height)
    ...
    def integrate(self, pano_depth_map, pano_rgb, mask, pose):
        ...
        for i in range(6):
            radial_depth = F.grid_sample(
                pano_depth_tensor, self.grids[i],
            ...
            # Compute face pose
            face_pose_mat = pose.clone()
            face_pose_mat[:3, :3] = pose[:3, :3] @ self.face_rotations[i]

            # Wrap as nvblox types (zero-copy)
            depth_image = DepthImage(optical_depth.contiguous())
            transform = Transform(face_pose_mat.contiguous())

            # Integrate using the new API
            self.mapper.integrate_depth(depth_image, transform, self.camera)

    def extract_mesh(self):
        """Generate mesh from the TSDF volume."""
        print("[TSDF] Generating dense planar mesh from nvblox...")
        self.mapper.update_mesh()
        return self.mapper.get_mesh()