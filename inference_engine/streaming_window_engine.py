...
class PanoStreamingEngine:
    ...
    def process_sequence(self, frames, masks):
        ...
        for i in range(0, num_frames - self.window_size + 1, self.window_size - self.overlap):
            ...
            torch.cuda.synchronize()  # Force wait for true NN inference time
            ...
            if self.is_first_window:
                ...
                torch.cuda.synchronize() # Wait for TSDF to truly finish
                print(f"  [Profile] TSDF Integration (Total for Window): {time.time() - t_tsdf_start:.4f} sec")
                ...
                print(f"  [Profile] Alignment & Odometry: {time.time() - t_align_start:.4f} sec")
            ...
            print(f"  [Profile] ----- Total Window {i} Time: {time.time() - t_win_start:.4f} sec -----")

        print(f"[Streaming Engine] Sequence processing complete in {time.time() - t_seq_start:.4f} sec.")
        
        # Notice we are safely passing the argument now!
