...
# Import SALAD image retrieval from your loop_closure.py
...
class PanoStreamingEngine:
    def __init__(self, vanilla_engine, window_size=16, overlap=2):
        ...
        # Initialize DINO-SALAD Retriever
        print("[SLAM] Initializing SALAD Loop Closure Model...")
        self.retriever = ImageRetrieval(input_size=224)
    ...
    def reset(self):
        ...
        # --- LOOP CLOSURE MEMORY ---
        self.submap_embeddings = {}
        self.submap_anchor_frames = {}
    ...
    def process_sequence(self, frames, masks):
        ...
        for i in range(0, num_frames - self.window_size + 1, self.window_size - self.overlap):
            ...
            # --- 4. GTSAM ODOMETRY & LOOP CLOSURE ---
            
            # Extract SALAD embedding for the middle frame of this submap
            mid_frame = window_frames[self.window_size // 2]
            current_emb = self.retriever.get_single_embeding(mid_frame)
            self.submap_embeddings[self.submap_count] = current_emb
            self.submap_anchor_frames[self.submap_count] = mid_frame
            ...
            if self.is_first_window:
                ...
                for old_id, old_emb in self.submap_embeddings.items():
                    # Must be at least 4 submaps ago (~12-20 frames) to be a true loop closure
                ...
                if best_score > 0.85: # Strict SALAD threshold
                    print(f"  [SLAM] 🟢 LOOP CLOSURE DETECTED: Submap {self.submap_count} -> {best_match_id} (Score: {best_score:.3f})")
                    
                    old_frame = self.submap_anchor_frames[best_match_id]
                    
                    # 1. Ask VGGT for the exact 3D spatial geometry between the two matching frames
                    # We pass the old frame and the current frame as a mini 2-frame sequence
                    lc_preds = self.engine(np.stack([old_frame, mid_frame]))
                    lc_poses = lc_preds["poses"]
                    
                    # 2. Extract the relative transform and apply the current metric scale
                    T_lc_raw = lc_poses[1] 
                    T_lc_metric = T_lc_raw.copy()
                    T_lc_metric[:3, 3] *= self.current_metric_scale
                    
                    # 3. Inject the loop closure edge into GTSAM!
                    self.pose_graph.add_loop_closure(from_id=best_match_id, to_id=self.submap_count, relative_mat=T_lc_metric)
                
                # Optimize graph (will seamlessly process both standard odometry AND loop closures)
                t_opt = time.time()
                self.pose_graph.optimize()
                print(f"  [Profile] GTSAM Optimization: {time.time() - t_opt:.4f} sec")

            # --- 5. FINALIZE GLOBAL POSES ---
            # Because GTSAM was just optimized, this anchor natively includes the loop closure correction!
