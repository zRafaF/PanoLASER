...
class StreamingWindowEngine:
    def __init__(self, vanilla_engine, window_size=16, overlap=2, device="cuda"):
        ...
        print("[Engine] Initializing Pure-PyTorch 3DGS Architecture (Nvblox Removed)...")
        self.reset()
    ...
    def process_sequence(self, frames, masks):
        ...
        for i in range(0, num_frames - self.window_size + 1, self.window_size - self.overlap):
            ...
            if len(batch_poses) > 0 and len(seed_pts) > 0:
                ...
                # SOTA Native Garbage Collector
                t_cull = time.time()
                killed = self.gs_mapper.garbage_collect(batch_depths, batch_masks, batch_poses)
                print(f"  > [Garbage Collector] Culled {killed} dynamic/ghost points in {time.time()-t_cull:.4f}s")
                
            profiler["Gaussian_Splat_&_Culling"] = time.time() - t3
            ...
            # --- ROBOT PATH PLANNING EXTRACTOR ---
            # We build an Open3D VoxelGrid instantly from the solid Gaussians.
            # Your path planning algorithm can query this grid to find empty routes!
            if len(self.last_pcd.points) > 0:
                self.collision_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(self.last_pcd, voxel_size=0.05)
